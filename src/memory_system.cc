...
bool MemorySystem::MaybeBroadcast(uint64_t cmd, uint64_t addr) {
  // TODO: add cmd interpretation and change cmd input type
  if (cmd >= static_cast<uint64_t>(CommandType::SIZE)) {
    fprintf(stderr,
            "Could not interpret command id %lu as valid command (must be "
            "less than %lu)",
            cmd, static_cast<uint64_t>(CommandType::SIZE));
    AbruptExit(__FILE__, __LINE__);
  }
  CommandType t = static_cast<CommandType>(cmd);
  Address a = config_->AddressMapping(addr);
  Command command = Command(t, a, addr);

  return dram_system_->MaybeBroadcast(command);
}
...
int MemorySystem::GetConfigParameter(std::string identifier) {
  ...
  if (identifier == "tRCDRD" || identifier == "tRCDWR") {
    std::cerr << "Failed to read config parameter " << identifier
              << " reason: not implemented." << std::endl;
    return -1;
  }
  if (identifier == "tRAS") {
    return config_->tRAS;
...
uint64_t MemorySystem::BankLocalToGlobalAddr(uint64_t channel, uint64_t rank,
  ...
  if ((hex_addr & local_mask) != hex_addr) {
    std::cerr << "Bank address out of bounds: " << std::hex << hex_addr
              << " (local address) > " << local_mask << " (maximum allowed)"
              << std::endl;
    AbruptExit(__FILE__, __LINE__);
  }
...
int MemorySystem::GetActiveRow(uint64_t channel, uint64_t rank,
                               uint64_t bankgroup, uint64_t bank) {
